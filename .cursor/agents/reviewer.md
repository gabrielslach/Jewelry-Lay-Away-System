...
3. Also check `.cursor/rules/code-quality.mdc`:
   ...
   - New or materially changed components/pages have colocated Vitest + Testing Library tests (`Foo.jsx` → `Foo.test.jsx`). Critical if missing, if tests skip user-visible behavior, or if `npm test` is failing.
